...
<script>
  ...
  export default {
    ...
    data () {
      return {
        departmentName: '',
        isShowTree: false, // 选中的部门
        selectDepartment: {},
        isShowOperationModal: false,
        isShowDeleteModal: false, // 部门参数
        departmentParam: {
          id: '',
          name: '',
          managerId: null,
        ...
        selectEmployeeId: null, // 备用的部门参数
        departmentParamBackup: {},
        isShowCompanySettingModal: false, // 员工table 表格样式功能
        departmentGroup: [{
          name: '',
          expand: true,
    ...
    methods: {
      ...
      async getListEmployeeByDepartmentName () {
        this.$Spin.show();
        let result = await departmentApi.getListEmployeeByDepartmentName(this.departmentName);
        this.$Spin.hide();
