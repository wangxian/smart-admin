                ...
                <Submenu :key="j" :name="children.menuKey" v-if="children.children.length > 0">
                  <template slot="title">
                    <template v-if="children.hideInMenu">
                      <Icon type="md-open"/>
                      <i style="font-size:0.85rem"> {{children.menuName}}</i>
                    </template>
                    <template v-else>
                      <Icon type="md-menu"/>
                      {{children.menuName}}
                    ...
                    <Submenu :key="k" :name="childrenPages.menuKey" v-if="childrenPages.children.length > 0">
                      <template slot="title">
                        <template v-if="childrenPages.hideInMenu">
                          <Icon type="md-open"/>
                          <i style="font-size:0.85rem"> {{childrenPages.menuName}}</i>
                        </template>
                        <template v-else>
                          <Icon type="md-menu"/>
                          {{childrenPages.menuName}}
                        </template>
                      </template>
                      <template v-for="(fourLevelItem,m) in childrenPages.children">
                        <MenuItem :key="m" v-if="fourLevelItem.hideInMenu" :name="hideInMenu.menuKey">
                          <Icon type="md-open"/>
                          <i style="font-size:0.85rem"> {{fourLevelItem.menuName}}</i>
                        </MenuItem>
                        <MenuItem :key="m" v-else :name="fourLevelItem.menuKey">
                          <Icon type="md-menu"/>
                          {{fourLevelItem.menuName}}
                        </MenuItem>
                      </template>
                    </Submenu>
    ...
    methods: {
      ...
      async buildPrivilegeTree () {
        ...
        if (privilegeList.length < serverMenuList.length) {
          ...
          this.menusChangeNum = this.menusChangeNum + Math.abs(privilegeList.length - serverMenuList.length);
        }
      ...
      hasMenuChange (menu, serverMenuMap) {
        ...
        if (serverMenu) {
          isChange = serverMenu.menuName !== menu.menuName || serverMenu.menuKey !== menu.menuKey || serverMenu.parentKey !== menu.parentKey || serverMenu.url !== menu.url;
        } else {
          isChange = true;
        }
      ...
      async addBatchSavePoints () {
        this.$Spin.show();
        let result = await privilegeApi.addBatchSavePoints(this.privilegeTableData.map(e => {
