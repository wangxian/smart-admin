...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      cacheList () {
        const list = ['ParentView', ...(this.tagNavList.length ? this.tagNavList.filter(item => !(item.meta && item.meta.noKeepAlive)).map(item => item.name): [])];
        return list;
      },
      local () {
    ...
    watch: {
      searchKeyWord (val) {
        if (val) {
          this.searchListResult = this.searchList.filter(item => item.title.indexOf(val) >= 0);
        }
      },
      $route (newRoute) {
    ...
    methods: {
      ...mapMutations(['setBreadCrumb', 'setTagNavList', 'pushKeepAliveIncludes', 'clearKeepAliveIncludes', 'deleteKeepAliveIncludes', 'deleteOtherKeepAliveIncludes', 'addTag', 'setLocal', 'setHomeRoute', 'closeTag']), ...mapActions(['handleLogin']),
      ...
      buildTopMenu () {
        ...
        for (let topMenu of topMenuArray) {
          if (this.$store.state.user.userLoginInfo.isSuperMan || this.$store.state.user.privilegeMenuKeyList.indexOf(topMenu.name) !== -1) {
            arr.push(topMenu);
          }
        }
        if (arr.length > 0) {
