...
<script>
  ...
  export default {
    ...
    data () {
      return {
        loading: false, // Websocket心跳间隔
        heartTimer: null, // 是否展示消息列表
        modalOpen: false,
        socketBaseUrl, // websocket 实例
        socket: null, // 是否展示消息详情
    ...
    methods: {
      initWebSocket () {
        this.socket = new WebSocket(`${this.socketBaseUrl}webSocket/${this.userInfo.id}`);
